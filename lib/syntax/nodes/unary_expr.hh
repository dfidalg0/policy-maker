...
namespace syntax {
    ...
    private:
        std::shared_ptr<syntax::Expr> _operand;
        OpKind _op;
    public:
        UnaryExpr(
            std::shared_ptr<syntax::Expr> operand,
            OpKind op,
            Position begin = Position(0, 0),
            Position end = Position(0, 0)
        ) : syntax::Expr(Node::Kind::unary_expr, begin, end), _operand(operand), _op(op) {}

        inline std::shared_ptr<syntax::Expr> operand() const { return _operand; }
