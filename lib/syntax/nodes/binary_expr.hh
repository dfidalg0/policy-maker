...
namespace syntax {
    ...
    private:
        std::shared_ptr<syntax::Expr> _left;
        std::shared_ptr<syntax::Expr> _right;
        OpKind _op;
    public:
        BinaryExpr(
        ...
        ) : syntax::Expr(Node::Kind::binary_expr, begin, end), _left(left), _right(right), _op(op) {}

        inline std::shared_ptr<syntax::Expr> left() const { return _left; }
        inline std::shared_ptr<syntax::Expr> right() const { return _right; }
